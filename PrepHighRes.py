...
DISPLAY_TITLE = r"""
      ...
      | |      | ___ \             | | | (_)     | |   | ___ \         
 _ __ | |______| |_/ / __ ___ _ __ | |_| |_  __ _| |__ | |_/ /___  ___ 
| '_ \| |______|  __/ '__/ _ \ '_ \|  _  | |/ _` | '_ \|    // _ \/ __|
| |_) | |      | |  | | |  __/ |_) | | | | | (_| | | | | |\ \  __/\__ \
| .__/|_|      \_|  |_|  \___| .__/\_| |_/_|\__, |_| |_\_| \_\___||___/
| |                          | |             __/ |                     
|_|                          |_|            |___/                      
"""

parser = ArgumentParser(description='A plugin to prepare the data for the training',
                        formatter_class=ArgumentDefaultsHelpFormatter)
parser = argparse.ArgumentParser('   ==========   Data preparation code for fetal Attention U-Net for CP segmentation on high resolution recontruction script    ==========   ')
...
@chris_plugin(
    parser=parser,
    title='My ChRIS Plugin to Prepare High Resolution MRIs',
...
#def main(input_data_path, output_data_path, mask_file, GA_info = None, input_size = 192, split_method = 'train_valid_split', n_fold = 5):
    ...
    MR_list = np.asarray(sorted(glob.glob(inputdir + '/Upsample_MR' + '/*.nii')))
    GT_list = np.asarray(sorted(glob.glob(inputdir+ '/Upsample_GT' + '/*.nii')))
